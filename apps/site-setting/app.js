module.exports = function init(site) {
  ...
  site.getMamaSetting = function (req) {
    let mamaId = null;
    ...
    if (setting.isShared) {
      if (req.session.user && req.session.user.type == "mama") {
        mamaId = req.session.user.id;
      } else if (req.session.selectedMamaId) {
        mamaId = req.session.selectedMamaId;
      }
    } else {
      if (setting.mama && setting.mama.id) {
        mamaId = setting.mama.id;
      }
    }
    return mamaId;
  };
  ...
  site.post({ name: "/api/set-site-setting", require: { permissions: ["login"] } }, (req, res) => {
    ...
    data.mamaId = data.mama && data.mama.id ? data.mama.id : 0;
    if (index > -1) {
